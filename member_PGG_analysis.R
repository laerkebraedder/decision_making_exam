...
pacman::p_load(R2jags, parallel, polspline, ggplot2, glue, hesim)
...
#### GROUP DIFF: DELTA ALPHA (= belief about initial group contribution)
...
lines(density(CC.samples$BUGSoutput$sims.list$delta_alpha),col="red")

fit.posterior <- logspline(CC.samples$BUGSoutput$sims.list$delta_alpha)
null.posterior <- dlogspline(0, fit.posterior)

null.prior     <- dnorm(0,0,(1/sqrt(1)))                   

BF_effect$delta_alpha <- null.prior/null.posterior
BF_null$delta_alpha <- null.posterior/null.prior

# estimating and plotting Bayesian credible interval (95%)
MAP_delta_alpha <- MPD(CC.samples$BUGSoutput$sims.list$delta_alpha)
MAP_delta_alpha

delta_alpha_cred = quantile(CC.samples$BUGSoutput$sims.list$delta_alpha,c(0.025,0.975))
delta_alpha_cred

# Plot density of posterior + credible interval (95%) incl. MAP (maximum a posteori probability)
plot(density(CC.samples$BUGSoutput$sims.list$delta_alpha), main=" ")
lines(delta_alpha_cred, c(0,0), col="red")
points(MAP_delta_alpha, 0, pch=19, col="red")


#### GROUP AVG: MU ALPHA LOG (= belief about initial group contribution)
...
#### GROUP DIFF: DELTA RHO (= degree of undermatching, aka. conditional cooperation)

# savage dickey plot
plot(density(rnorm(30000,0,1/sqrt(1))),ylim=c(0,1),main=" ")
lines(density(CC.samples$BUGSoutput$sims.list$delta_rho),col="red")

fit.posterior <- logspline(CC.samples$BUGSoutput$sims.list$delta_rho)
null.posterior <- dlogspline(0, fit.posterior)

null.prior     <- dnorm(0,0,(1/sqrt(1)))                   

BF_effect$delta_rho <- null.prior/null.posterior
BF_null$delta_rho <- null.posterior/null.prior



#### GROUP AVG: MU RHO PROBIT

BF_effect <- NULL
BF_null <- NULL

# savage dickey plot
plot(density(rnorm(30000,0,1/sqrt(1))),ylim=c(0,5.7),main=" ")
...
#### GROUP DIFF: DELTA OMEGA (= attention to others, i.e. how much weight on people's previous contribution compared to initial belief)

# savage dickey plot
plot(density(rnorm(30000,0,1/sqrt(1))),ylim=c(0,1),main=" ")
lines(density(CC.samples$BUGSoutput$sims.list$delta_omega),col="red")

fit.posterior <- logspline(CC.samples$BUGSoutput$sims.list$delta_omega)
null.posterior <- dlogspline(0, fit.posterior)

null.prior     <- dnorm(0,0,(1/sqrt(1)))                   

BF_effect$delta_omega <- null.prior/null.posterior
BF_null$delta_omega <- null.posterior/null.prior



#### GROUP AVG: MU Omega PROBIT

BF_effect <- NULL
BF_null <- NULL

# savage dickey plot
plot(density(rnorm(30000,0,1/sqrt(1))),ylim=c(0,1.5),main=" ")
...
##### DELTA alpha (from normal to gamma distr - via log (inv log > exp()))
# estimating and plotting Bayesian credible interval (95%)
MAP_delta_alpha <- MPD(exp(CC.samples$BUGSoutput$sims.list$delta_alpha))
MAP_delta_alpha

delta_alpha_cred = quantile(exp(CC.samples$BUGSoutput$sims.list$delta_alpha),c(0.025,0.975))
delta_alpha_cred

# Plot density of posterior + credible interval (95%) incl. MAP (maximum a posteori probability)
plot(density(exp(CC.samples$BUGSoutput$sims.list$delta_alpha)), 
     main = "Group DIFF alpha (reparameterized) - belief about initial contrib")
lines(delta_alpha_cred, c(0,0), col="red")
points(MAP_delta_alpha, 0, pch=19, col="red")

##### GROUP AVG: MU ALPHA LOG (from normal to gamma distr - via log (inv log > exp()))
# estimating and plotting Bayesian credible interval (95%)
MAP_alpha <- MPD(exp(CC.samples$BUGSoutput$sims.list$mu_alpha_log))
MAP_alpha

alpha_cred = quantile(exp(CC.samples$BUGSoutput$sims.list$mu_alpha_log),c(0.025,0.975))
alpha_cred

# Plot density of posterior + credible interval (95%) incl. MAP (maximum a posteori probability)
plot(density(exp(CC.samples$BUGSoutput$sims.list$mu_alpha_log)), 
     main = "Group AVG alpha (reparameterized) - belief about initial contrib")
...
#### GROUP DIFF: DELTA RHO (= degree of undermatching, aka. conditional cooperation)
# estimating and plotting Bayesian credible interval (95%)
MAP_delta_rho <- MPD(pnorm(CC.samples$BUGSoutput$sims.list$delta_rho))
MAP_delta_rho

delta_rho_cred = quantile(pnorm(CC.samples$BUGSoutput$sims.list$delta_rho),c(0.025,0.975))
delta_rho_cred

# Plot density of posterior + credible interval (95%) incl. MAP (maximum a posteori probability)
plot(density(pnorm(CC.samples$BUGSoutput$sims.list$delta_rho)), 
     main = "Group DIFF rho (reparameterized) - conditional cooperation")
lines(delta_rho_cred, c(0,0), col="red")
points(MAP_delta_rho, 0, pch=19, col="red")

#### GROUP AVG: MU RHO PROBIT (= degree of undermatching, aka. conditional cooperation)
# estimating and plotting Bayesian credible interval (95%)
MAP_mu_rho_probit <- MPD(pnorm(CC.samples$BUGSoutput$sims.list$mu_rho_probit))
MAP_mu_rho_probit

mu_rho_probit_cred = quantile(pnorm(CC.samples$BUGSoutput$sims.list$mu_rho_probit),c(0.025,0.975))
mu_rho_probit_cred

# Plot density of posterior + credible interval (95%) incl. MAP (maximum a posteori probability)
plot(density(pnorm(CC.samples$BUGSoutput$sims.list$mu_rho_probit)), 
     main = "Group AVG rho (reparameterized) - conditional cooperation")
lines(mu_rho_probit_cred, c(0,0), col="red")
points(MAP_mu_rho_probit, 0, pch=19, col="red")
...
#### GROUP DIFF: DELTA OMEGA (= attention to others, i.e. how much weight on people's previous contribution compared to initial belief)
# estimating and plotting Bayesian credible interval (95%)
MAP_delta_omega <- MPD(pnorm(CC.samples$BUGSoutput$sims.list$delta_omega))
MAP_delta_omega

delta_omega_cred = quantile(pnorm(CC.samples$BUGSoutput$sims.list$delta_omega),c(0.025,0.975))
delta_omega_cred

# Plot density of posterior + credible interval (95%) incl. MAP (maximum a posteori probability)
plot(density(pnorm(CC.samples$BUGSoutput$sims.list$delta_omega)), 
     main = "Group DIFF omega (reparameterized) - attention to others")
lines(delta_omega_cred, c(0,0), col="red")
points(MAP_delta_omega, 0, pch=19, col="red")

#### GROUP AVG: MU OMEGA PROBIT (= degree of undermatching, aka. conditional cooperation)
# estimating and plotting Bayesian credible interval (95%)
MAP_mu_omega_probit <- MPD(pnorm(CC.samples$BUGSoutput$sims.list$mu_omega_probit))
MAP_mu_omega_probit

mu_omega_probit_cred = quantile(pnorm(CC.samples$BUGSoutput$sims.list$mu_omega_probit),c(0.025,0.975))
mu_omega_probit_cred

# Plot density of posterior + credible interval (95%) incl. MAP (maximum a posteori probability)
plot(density(pnorm(CC.samples$BUGSoutput$sims.list$mu_omega_probit)), 
     main = "Group AVG omega (reparameterized) - attention to others")
lines(mu_omega_probit_cred, c(0,0), col="red")
points(MAP_mu_omega_probit, 0, pch=19, col="red")
