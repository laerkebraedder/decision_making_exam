CC_member <- function(groupSize, ntrials, ngroups, rate_alpha, shape_alpha,
  ...
  for (g in 1:ngroups) {
    ...
    for (s in 1:groupSize) {
      ...
      if (membership[s,g,1]) {
        alpha_s[s,g] <- rgamma(1,shape_alpha_member,rate_alpha_member)
        rho_s[s,g] <- rbeta(1,shape1_rho_member,shape2_rho_member)
        omega_s[s,g]  <- rbeta(1,shape1_omega_member,shape2_omega_member)
      }
      else if (!membership[s,g,1]) {
